...
- 病害的描述：
    ...
    - 马蹄的时候：./templates/图例/裂缝及其长宽.dxf ，它就是线。所有的病害都用线表达。
...
- T梁的坐标单位
  x方向：1m=10坐标。
  y方向：2.25m=21.9坐标,换算一下就是1m=9.73坐标。从上到下。

- T梁的标注文字和线段的方向
  - 左翼缘板：向下45度。
    - 标注线段起始位置 x> 450, 标注的线段 225度。
  - 右翼缘板：向上45度。
    - 标注线段起始位置 x> 450, 标注的线段 225度。
  - 左腹板：
    - 上部T梁：
        - 标注线段其实位置 x < 100 
            - y > 273标注的线 ，315度
            - y < 273标注的线 ，45度
        - 标注线段其实位置 x > 100 并且 x < 450
            - y > 273标注的线 ，225度 或者 315度
            - y < 273标注的线 ，45度 或者 135度
        - 标注线段起始位置 x> 450 
            - y > 273标注的线段 225度。
            - y < 273标注的线段 135度。
    - 下部T梁：
        - 标注线段其实位置 x < 100 
            ...
            - y < 161标注的线 ，45度
        - 标注线段其实位置 x > 100 并且 x < 450
            - y > 161标注的线 ，225度 或者 315度
            - y < 161标注的线 ，45度 或者 135度
        - 标注线段起始位置 x> 450 
            - y > 161标注的线段 225度。
            - y < 161标注的线段 135度。
  - 右腹板：
   - 上部T梁：
        - 标注线段其实位置 x < 100 
            ...
            - y < 245标注的线 ，45度
        - 标注线段其实位置 x > 100 并且 x < 450
            - y > 245标注的线 ，225度 或者 315度
            - y < 245标注的线 ，45度 或者 135度
        - 标注线段起始位置 x> 450 
            - y > 245标注的线段 225度。
            - y < 245标注的线段 135度。
    - 下部T梁：
        - 标注线段其实位置 x < 100 
            ...
            - y < 132标注的线 ，45度
        - 标注线段其实位置 x > 100 并且 x < 450
            - y > 132标注的线 ，225度 或者 315度
            - y < 132标注的线 ，45度 或者 135度
        - 标注线段起始位置 x> 450 
            - y > 132标注的线段 225度。
            - y < 132标注的线段 135度。
  - 马蹄左侧面：向上45度。
  ...
  - 第一条引线的长度
   - 
    合理安排水平线，保证文字不会被两段引线交叉。另外。水平线过长。因该根据文字长短合理调整。还有就是第一段引线因该紧挨着图例，并且右下倾斜的时候就挨着图例的右下角。右上倾斜的时候就挨着图例的右上角。左下倾斜的时候就挨着图例的左下角。同样左上倾斜的时候就在这图例的左上角。

  - 引线的横线的方向： 向哪里倾斜，水平横线就向哪里。
        右上倾斜（45°）：斜线向右上方走 → 水平线应该向右延伸
        左上倾斜（135°）：斜线向左上方走 → 水平线应该向左延伸
        左下倾斜（225°）：斜线向左下方走 → 水平线应该向左延伸
        右下倾斜（315°）：斜线向右下方走 → 水平线应该向右延伸
## 技术要求
- 使用python
- 客户使用浩辰CAD软件
